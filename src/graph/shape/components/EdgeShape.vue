...
import { computed, inject, toRaw, onMounted } from "vue";
...
const labelStyle = computed(() => {
  const {
    bounds: { absX: parentAbsX, absY: parentAbsY },
    waypoint,
  } = props.shape;
  ...
  if (editable.value) {
    return {
      ...
      absX:
        parentAbsX +
        (lastPoint.x - firstPoint.x) * offsetX -
        width / 2 -
        paddingWidth / 2,
...
const handleInput = (e) => {
  ...
  const {
    nameBounds: { offsetX },
    bounds: { width: parentWidth, absX },
  } = props.shape;
  const { width, height } = shapeUtil.getTextSize(
    text.value,
    props.shape.nameStyle.fontSize
  );
  updateShapeService(
    props.shape.id,
    {
...
const handleBlur = () => {
  ...
  if (prevText.value !== text.value) {
    updateShapeService(
      props.shape.id,
      { nameBounds: props.shape.nameBounds, modelName: text.value },
      { nameBounds: prevNameBounds.value, modelName: props.shape.modelName }
    );
